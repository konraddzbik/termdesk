...
`AGENT_TOOLS` maps each drivable primitive (`session.open/run/read`, `sftp.get/put`, `tunnel.start/stop`, `fleet.run`) to a descriptor; `requiresApproval(name, opts)` is the pure policy: **fail-closed on unknown tools**, every mutating tool always gated, and only the read-only `session.read` is eligible for an opt-in auto-approve. **Integration:** register these tools in `src/main/mcp/` and log each call to `aiAudit`. `requiresApproval` is an **additional** gate layered on the shipped host-scoped policy (`decide()` in `src/main/mcp/policy.ts`), never a replacement: `decide()` runs first, so the per-host read/exec allow-sets and the built-in deny list stay binding. Combine them with `gateAgentCall(decide(...).verdict, tool, opts)`, where the stricter answer wins. A `deny` is final. `needs-approval` stays. An `allow` (read-enabled host, or an allowlist-mode pattern match) becomes a prompt whenever `requiresApproval` says so. So an allowlist match never auto-runs a mutating agent tool, and `autoApproveReads` only skips the prompt on a host `decide()` already read-enabled — credentials never leave main, exactly as today. Long-running commands stay observable (the agent polls `session.read`) without blocking approvals.

## #97 — Inline preview detection (this PR's core)

`detectPreview(content)` inspects at most a bounded prefix (`maxChars`, default 256 KiB; truncated input is never JSON-parsed) and classifies output as `image` (data-URI or base64-signature), `json` (object/array), `table` (CSV/TSV with column+row counts), or `text`, best-effort and side-effect-free, in an order that won't mistake a multi-line JSON array for CSV. **Integration:** the renderer (`components/terminal`, `components/sftp`) draws the preview inline via the M6 image path for images. **SVG must be rendered via `<img>`, never inlined into the DOM**, because inline SVG can run script (XSS) and lightweight table/tree views for data; an SFTP "quicklook" streams a bounded prefix so a huge file can't hang the renderer. Previews attach to the command block that produced them, giving the #95 agent tools a structured artifact to reference/re-run.
...
`agent-tools.test.ts`: registry invariants (every mutating tool gated; `session.read` the only read-only); `requiresApproval` always gates mutating tools and fails closed on unknown, relaxes reads only on opt-in, exact-case names; `gateAgentCall` never lifts a `deny` and downgrades allowlist `allow` for mutating tools; frozen registry; `detectPreview` for data-URI + bare-base64 images, JSON object/array, JSON-array-not-CSV, CSV/TSV with counts, and text fallbacks (prose, single line, empty, inconsistent delimiters), `sampleLines`, the `maxChars` bound, RIFF-but-not-WEBP. `lint` / `typecheck` / `test` / `build` green. No MCP runtime, no network, no secret flow in the core.
